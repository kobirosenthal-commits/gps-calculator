...
from credit_scoring import (assess_application, search_customers, refresh_source_index,
                            SOURCE_DIRECTORY, INDEX_DATABASE)
import credit_compliance as compliance
import credit_auth as auth
import credit_validation as validation
import credit_portfolio as portfolio
...
def require_auth(role='underwriter'):
    """Session-based auth gate. APIs get JSON 401/403; pages redirect."""
    def decorator(view):
        @wraps(view)
        def wrapped(*args, **kwargs):
            ...
            if not user:
                if request.path.startswith('/api/'):
                    return jsonify({'error': 'נדרשת התחברות', 'error_code': 'AUTH_REQUIRED'}), 401
                return redirect(url_for('login', next=request.path))
            if not auth.has_role(user['role'], role):
                return jsonify({'error': 'אין הרשאה מתאימה לפעולה זו',
...
def _sqlite_ready(path, required_table):
    try:
        import sqlite3
...
def _source_file_count():
    try:
        files = [p for p in Path(SOURCE_DIRECTORY).iterdir()
...
def users_admin():
    if request.method == 'GET':
        return jsonify({'users': auth.list_users(),
                        'auth_events': auth.auth_events(50)})
    payload = request.get_json(silent=True) or {}
    try:
        created = auth.create_user(payload.get('username'), payload.get('password'),
                                   payload.get('role'), payload.get('display_name'))
...
def favicon():
    return app.send_static_file('creditlens.ico')


@app.route('/healthz')
def healthz():
    """Liveness probe: process responds and app routes are loaded."""
    return jsonify({
        'status': 'ok',
        'service': 'creditlens',
...
@app.route('/readyz')
def readyz():
    """Readiness probe: dependencies are available for serving requests."""
    index_refresh_ok = True
    try:
        refresh_source_index()
    except Exception:
        index_refresh_ok = False
    checks = {
        'index_refresh': index_refresh_ok,
        'source_directory': Path(SOURCE_DIRECTORY).exists(),
        'source_files': _source_file_count(),
        'auth_db': _sqlite_ready(auth.AUTH_DATABASE, 'users'),
        'compliance_db': _sqlite_ready(compliance.COMPLIANCE_DATABASE, 'decisions'),
        'index_db': _sqlite_ready(INDEX_DATABASE, 'customer_records'),
        'validation_db': _sqlite_ready(validation.VALIDATION_DATABASE, 'validation_runs'),
    }
    ready = checks['source_directory'] and checks['source_files'] > 0 and all(
        checks[key] for key in (
            'index_refresh', 'auth_db', 'compliance_db', 'index_db', 'validation_db'
        )
    )
    payload = {
        'ready': ready,
...
def credit_assess():
    ...
    operator = session['user']['username']  # audit identity from the login, not the client

    # Regulatory gate: assessment requires documented customer consent.
    consent = compliance.get_active_consent(customer_id)
    if consent is None:
        if payload.get('consent_given'):
            consent = compliance.record_consent(customer_id, operator)
        else:
            return jsonify({
                'error': 'נדרשת הסכמת לקוח מתועדת לפני שליפת נתוני אשראי',
                'error_code': 'CONSENT_REQUIRED',
            }), 428

    try:
        amount = float(payload.get('amount', 100000))
        result = assess_application(customer_id, amount)
    except LookupError as exc:
        return jsonify({'error': str(exc)}), 404
    except (TypeError, ValueError) as exc:
        return jsonify({'error': str(exc)}), 400

    decision_id = compliance.record_decision(result, operator, consent['consent_id'])
...
@app.route('/api/credit/consent', methods=['POST'])
@require_auth('underwriter')
def credit_consent():
    payload = request.get_json(silent=True) or {}
    customer_id = (payload.get('customer_id') or '').strip()
    if not customer_id:
        return jsonify({'error': 'יש לציין מספר לקוח'}), 400
    record = compliance.record_consent(customer_id, session['user']['username'])
    return jsonify(record), 201


@app.route('/api/credit/decisions', methods=['GET'])
@require_auth('risk_manager')
def credit_decisions():
    try:
        limit = min(int(request.args.get('limit', 50)), 200)
    except ValueError:
        limit = 50
    return jsonify({
        'decisions': compliance.decision_history(
            request.args.get('customer_id') or None, limit),
    })


@app.route('/api/credit/compliance/policy', methods=['GET'])
@require_auth('risk_manager')
def credit_compliance_policy():
    return jsonify(compliance.compliance_policy())
...
@app.route('/api/credit/portfolio', methods=['GET'])
@require_auth('risk_manager')
def credit_portfolio_report():
    try:
        days = int(request.args.get('days', 90))
    except ValueError:
        days = 90
    return jsonify(portfolio.portfolio_summary(days))


@app.route('/api/credit/decisions.csv', methods=['GET'])
@require_auth('risk_manager')
def credit_decisions_csv():
    try:
        days = int(request.args.get('days', 365))
    except ValueError:
        days = 365
    csv_text = portfolio.decisions_csv(request.args.get('customer_id') or None, days)
    stamp = datetime.now(timezone.utc).strftime('%Y%m%d-%H%M')
    return Response(
        '\ufeff' + csv_text,  # BOM so Excel opens Hebrew correctly
        mimetype='text/csv; charset=utf-8',
        headers={'Content-Disposition': f'attachment; filename=creditlens-decisions-{stamp}.csv'},
    )
...
def credit_batch():
    payload = request.get_json(silent=True) or {}
    customer_ids = payload.get('customer_ids') or None
    if isinstance(customer_ids, str):
        customer_ids = [part for part in re.split(r'[\s,;]+', customer_ids) if part]
    try:
        report = portfolio.batch_prescreen(
            customer_ids,
            float(payload.get('amount', 100000)),
            int(payload.get('sample', 200)),
        )
    except (TypeError, ValueError) as exc:
        return jsonify({'error': str(exc)}), 400
...
@require_auth('underwriter')
def decision_letter(decision_id):
    record = compliance.get_decision(decision_id)
    if record is None:
        return render_template('decision_letter.html', decision=None), 404
    signer = next((user['display_name'] for user in auth.list_users()
                   if user['username'] == record['operator']), record['operator'])
    return render_template('decision_letter.html', decision=record, signer=signer)
...
@app.route('/api/validation/latest', methods=['GET'])
@require_auth('risk_manager')
def validation_latest():
    report = validation.latest_report()
    if report is None:
        return jsonify({'error': 'טרם הופק דוח תיקוף'}), 404
    return jsonify(report)
...
def validation_run():
    try:
        report = validation.run_validation(session['user']['username'])
...
def validation_history():
    return jsonify({'runs': validation.validation_history()})


@app.route('/api/credit/sources', methods=['GET'])
@require_auth('underwriter')
def credit_sources():
    return jsonify({
        ...
        'message': 'המערכת סורקת את כל המקורות אוטומטית בעת כל בקשת הלוואה',
    })


@app.route('/api/credit/customers', methods=['GET'])
@require_auth('underwriter')
def credit_customers():
    ...
    try:
        ...
        limit = 50
    return jsonify(search_customers(query, limit))


@app.route('/calculator')
def calculator():
    # The classic calculator UI was replaced by CreditLens; send to 3D view.
